...
public class Game {
    ...
    public String checkGameWinner(char [][]grid) {
       ...
       String [] winner = {"X wins", "O wins"};
        ...
        for (int rc = 0; rc < grid.length; rc++) {   //rc: r->row and c->column
            for(int a=0;a<symbol.length;a++) {
                //Winning logic for row
                if (grid[rc][0] == grid[rc][1] && grid[rc][1] == grid[rc][2] && grid[rc][2] == symbol[a]) {
                    return winner[a];
                }

                //Winning logic for coloumn
                if( grid[0][rc]==grid[1][rc] && grid[1][rc]==grid[2][rc] && grid[2][rc]==symbol[a]){
                    return  winner[a];
                }
            }
        }

        //Winning logic for diagonal
        for (int i=0;i<symbol.length;i++) {
            if ((grid[0][0] == grid[1][1] && grid[1][1] == grid[2][2] && grid[2][2] == symbol[i]) ||
                    (grid[0][2] == grid[1][1] && grid[1][1] == grid[2][0] && grid[2][0] == symbol[i])) {
                return winner[i];
            }
        }
